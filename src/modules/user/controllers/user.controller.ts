...
import { AdminLoginDto } from 'src/modules/user/dtos/admin.login.dto';
...
export class UserController {
    ...
    @Post('/applogin')
    async appLogin(
        @Body() { email, password, rememberMe }: AdminLoginDto
    ): Promise<IResponse> {
        const user: IUserEntity =
            await this.userService.findOneByEmail<IUserEntity>(email, {
                join: true,
            });
        if (!user) {
            throw new NotFoundException({
