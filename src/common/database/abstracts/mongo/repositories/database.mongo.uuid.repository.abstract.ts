    ...
    implements IDatabaseRepository<T>
{
    ...
    async findglAccount<Y = T>(
        ...
        if (find.Type === 'Income') {
            const glAccounts = await findAll.lean();
            let glAccountsData = glAccounts.map((glAccount: any) => {
                return {
                    id: glAccount.Id,
                    name: glAccount.Name,
                    type: glAccount.Type,
            ...
            return glAccountsData;
        } else if (find.Type === 'Expense') {            
            const sub = [];
            const account: any = await findAll.lean();
            ...
            //                 subType: subAccount.SubType,
            //             });
            //         });
            //     }
            // });
            return account
        }  else if (find.Type === 'Asset') {
           let assets = await this._repository.find({Type:find.Type})
           return assets
        }
    }
